...
class Functions {
	 ...
	 */
	 function sendSms($phone,$sms_body){    
		...
		$fields = array(
					'method' => 'sms',
					'to' => $phone,
					'api_key' => 'A150f83bff7c28bf305c4aa869e900e5d',
					'sender' => 'rocket',
					'message' => $sms_body,
					'format' => 'json'
				);
	 ...
	 function addToLog( $to, $body, $isSent='Yes' ){
	 	$sql    = "INSERT INTO ".SMS_LOG." SET to_number='".$to."', sms_body='".addslashes($body)."',sms_sent='".$isSent."'";	 	
    ...
    function addEmailLog($to,$subject,$body){
    	$sql	=	"INSERT INTO ".EMAIL_LOG." SET 
					sent_to=".$this->obj->db->escape($to).",
					subject=".$this->obj->db->escape($subject).",
					body   =".$this->obj->db->escape($body);
     ...
     * @return boolean
     */
    function send_emailBKP($to_email,$template_code,$search_text=array(),$replace_text=array(),$subject='',$name='',$email='')
    {
    	$sql = "SELECT from_name, from_email, subject, email_body FROM ".EMAIL_TEMPLATE_SETTING." WHERE template_code = '".$template_code."'" ; //echo $sql;
	...
	function send_email($to_email,$template_code,$search_text=array(),$replace_text=array(),$subject='',$name='',$email='') {
    	...
    	$sql = "SELECT from_name, from_email, subject, email_body FROM ".EMAIL_TEMPLATE_SETTING." WHERE template_code = '".$template_code."'" ;
    	...
    	if(count($search_text)>0){
    		...
    		$msg = str_replace($search_text,$replace_text,$rs['email_body']);
    	}else{
    		$msg = $rs['email_body'];
    	}
    		...
    		$path 		=  front_base_url();
    		$footer	 	=  nl2br(GLOBAL_EMAIL_SIGNATURE);
    		$emailBody  = $this->emailTemplate($msg,$footer,$path);
    		$config 	= array(); 
			$config['api_key']= "key-8b6aeef7fb52523c8e13735ea059c92d"; 
			$config['api_url']= "https://api.mailgun.net/v3/rocket.co.in/messages"; 
			
			$message 				= array(); 
			$message['from'] 		= "rocket < feedback@rocket.co.in>";; 
			$message['to'] 			= $to_email; 
			$message['h:Reply-To'] 	= "rocket < feedback@rocket.co.in>";;
			$message['subject'] 	= $rs['subject']; 
   ...
   function module($module_id = '') {
        if (!empty($module_id)) {
            $query = $this->obj->db->get_where('tbl_modules', array('id' => $module_id));
            return $query->result_array();
        } else {
            $query = $this->obj->db->get_where('tbl_modules', array());
            return $query->result_array();
        }
    }
    /**
...
//            $this->nsession->set_userdata('user_session_user_type', $row->user_type);
	...
	**/
	...
	/**
	* Function for get pincode by lat/long from google API
	* @param string $latLong { lat,long }
	* @return string
	* @Gopal
	**/
	function getPincodeByLatLong($latLong) {
		$key = $this->googleAPIKey;
		...
		//pr($result);	
		$address = $postcode = "";
		if(isset($result['results'][0]['address_components']) && $result['status']=='OK'){
		...
		return array('address'=>$address,'postcode'=>$postcode);
	}
	...
	**/
    ...
    // to find address in guwahati
    function determine_address_in_polygon_guwahati($got_this_address) {
    	//$got_this_address	=	urldecode($got_this_address);
    	$polygons  = json_decode(POLYGON_GUWAHATI);  	 
     ...
     function determine_address_in_polygon($got_this_address) {
       ...
       // $address = file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.trim($got_this_address).',&sensor=true_or_false&key=GoogleAPIKey');
        $address   = json_decode($this->get_address_coordinate($got_this_address));      
        $address_x = 0;
        $address_y = 0;      
     ...
     function determine_address_in_polygon_all($got_this_address) {
        //$got_this_address =   urldecode($got_this_address);
        $polygons  = json_decode(POLYGON_ALL);      
 ...
 function determine_address_in_polygon_all_region($got_this_address) {
        //$got_this_address =   urldecode($got_this_address);
        $polygons  = json_decode(POLYGON_ALL_REGION);      
    ...
    function determine_in_polygon($no_of_polygons, $polygons_x, $polygons_y, $address_x, $address_y) {
    	...
    	*/
    	...
    	public function get_address_coordinate($address) {
          ...
          //  $url="https://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$long."&location_type=ROOFTOP&result_type=street_address&key=".$key;
         ...
         public function is_address_coordinate($lat,$long) {
            $key    = "AIzaSyBGyLsC9P94sx7I8hA3SSuzDSVqjPSM-4Q";
            //$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".$address."&sensor=false&key=".$key;
            
            $url="https://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$long."&location_type=ROOFTOP&result_type=street_address&key=".$key;

            // $url1    = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($address)."&sensor=false&key=".$key;
	...
	function dateTimeDiff( $interval='i', $dt1, $dt2, $relative=false){	   
	 ...
	 function rating($rate){
		for($j=0;$j<$rate;$j++){
			echo '<span><i class="glyphicon glyphicon-star yellow"></i></span>';
		}
		for($j=0;$j<5-$rate;$j++){
			echo '<span><i class="glyphicon glyphicon-star-empty"></i></span>';
		}
	 }
	 
	/**
	 * Function used for get rate amount for INVOICE
	 * @param int $amount
	 * @param string $invoiceDate
	 * @return number
	 */ 
	function getRateAmount($amount,$invoiceDate){
		if($invoiceDate>'2016-05-31'){
			$rate_amount = (100/115)* $amount;
		}else{
			$rate_amount = (100/114.50)* $amount;			
		}
	    return round($rate_amount,2);
	}
	/**
	 * Function for get service tax for INVOICE
	 * @param int $amount
	 * @param string $invoiceDate
	 * @return number
	 */
	function getStaxAmount($amount,$invoiceDate)	{
		if($invoiceDate>'2016-05-31'){
			$rate_amount = (100/115)* $amount;
		}else{
			$rate_amount = (100/114.50)* $amount;			
		}
			$stax_amount = $rate_amount*(14/100);
	 return round($stax_amount,2);
	}
	
	/**
	 * Function for get Swachh Bharat Cess tax
	 * @param int $amount
	 * @param string $invoiceDate
	 * @return number
	 */
	function getSBCtaxAmount($amount,$invoiceDate){
		if($invoiceDate>'2016-05-31'){
			$rate_amount = (100/115)* $amount;
		}else{
			$rate_amount = (100/114.50)* $amount;			
		}
		$sbctax_amount = $rate_amount*(0.50/100);
		return round($sbctax_amount,2);
	}
	/**
	 * Function for get Krishi Kalyan Cess tax
	 * @param int $amount
	 * @param string $invoiceDate
	 * @return number
	 */
	function getKKCtaxAmount($amount,$invoiceDate){
		if($invoiceDate>'2016-05-31'){
			$rate_amount = (100/115)* $amount;
		}else{
			$rate_amount = (100/114.50)* $amount;			
		}
		$kkctax_amount 	 = $rate_amount*(0.50/100);
		return round($kkctax_amount,2);
	}
        
         function getvehicleclass($id = '') {
        if (!empty($id)) {
            $query = $this->obj->db->get_where('tbl_vehicle', array('deleted' => '0', 'country_id' => $id, 'status' => 'Active'));
            return $query->result_array();
        } else {
            $query = $this->obj->db->get_where('tbl_vehicle', array('deleted' => '0', 'status' => 'Active'));
            return $query->result_array();
        }
    }
    
    
     function get_customer_id_from_order($order_id = '') {
    
        if (!empty($order_id)) {
            $query = $this->obj->db->get_where('tbl_order_master', array('is_deleted' => '0', 'order_id' => $order_id));
    
            return $query->result_array();
        } else {
            $query = $this->obj->db->get_where('tbl_order_master', array('is_deleted' => '0'));
            return $query->result_array();
        }
    }
       ...
       function vehicle_type($id = '') {
        ...
        if (!empty($id)) {
            $query = $this->obj->db->get_where('tbl_vehicle', array('deleted' => '0', 'id' => $id, 'status' => 'Active'));
            return $query->result_array();
        } else {
            $query = $this->obj->db->get_where('tbl_vehicle', array('deleted' => '0', 'status' => 'Active'));
            return $query->result_array();
    ...
    function bankSet($id = '') {
        ...
        if (!empty($id)) {
            $query = $this->obj->db->get_where('tbl_bank_name', array('deleted' => '0', 'id' => $id, 'status' => 'Active'));
    
            return $query->result_array();
        } else {
            $query = $this->obj->db->get_where('tbl_bank_name', array('deleted' => '0', 'status' => 'Active'));
            return $query->result_array();
        }
    }
